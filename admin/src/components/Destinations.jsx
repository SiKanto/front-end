    ...
    async function fetchDestinations() {
      try {
        const response = await axios.get(
          "https://ml-kanto.up.railway.app/destinations"
        );
        ...
        if (Array.isArray(data.destinations)) {
          ...
          const existingDestinationsResponse = await axios.get(
            "https://kanto-backend.up.railway.app/destinations"
          );
          const existingDestinations = existingDestinationsResponse.data;

          const newDestinations = fetchedDestinations.filter(
            (dest) =>
              !existingDestinations.some((exist) => exist.name === dest.name)
          );

          if (newDestinations.length > 0) {
            await axios.post(
              "https://kanto-backend.up.railway.app/add-destinations",
              {
          ...
          console.error(
            "Destinations data is not an array:",
            data.destinations
          );
        }
      } catch (error) {
